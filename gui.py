...
def initialize_gui():
    ...
    try:
        ...
        upload_image_entry.bind("<Button-1>", lambda event: image_search(upload_image_entry))
        ...
        messagebox.showerror("Surf", "Failed to launch search window")
...
def display_results():
    ...
    try:
        ...
        if not search_results:
            messagebox.showinfo("Surf", "No search results found")
            return
        else:
            ...
            label = ttk.Label(frame, text = f"Search Results ({result_count} Found, {search_time:.2f} Seconds)", font = ("Helvetica", 20, "bold"))
            label.pack(padx = 5)
            ...
            for result in search_results:
                ...
                engine_label = ttk.Label(result_frame, text = result["engine"], font = ("Helvetica", 16, "bold"))
                engine_label.pack(anchor = "e")
        ...
        messagebox.showerror("Surf", "Failed to display search results")
...
def clear_results():
    try:
        ...
        messagebox.showinfo("Surf", "No search results to clear")
...
def refresh_results():
    try:
        ...
        messagebox.showinfo("Surf", "No search results to refresh")
